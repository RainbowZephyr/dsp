...
class DualSincFilter
  getter :order, :sample_rate, :left_cutoff, :right_cutoff, :window_class, :left_filter,
    :right_filter, :bandpass_fir, :bandstop_fir
  ...
  def initialize(order : Int32, sample_rate : Float64, left_cutoff : Float64, right_cutoff : Float64, @window_class : Window.class)
    ...
    @left_filter = SincFilter.new(order: order, cutoff: left_cutoff,
      sample_rate: sample_rate, window_class: window_class)
    @right_filter = SincFilter.new(order: order, cutoff: right_cutoff,
      sample_rate: sample_rate, window_class: window_class)
