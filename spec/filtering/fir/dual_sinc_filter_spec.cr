...
describe DualSincFilter do
  [140,200].each do |order|
    [{300.0,400.0}, {500.0,700.0}, {800.0,1000.0}].each do |left_cutoff, right_cutoff|
      center = (left_cutoff + right_cutoff) / 2.0
      bandwidth = right_cutoff - left_cutoff

      filter = DualSincFilter.new(order: order, left_cutoff: left_cutoff,
        right_cutoff: right_cutoff, sample_rate: sample_rate, window_class: Window::Blackman)
